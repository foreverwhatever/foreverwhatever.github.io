...
import IPython, pandas, sklearn.tree, types
__all__ = ['Describer', 'nx_tree', 'procedure']


# In[11]:

def procedure(composed_function):
    """Describe a higher-order function from its docstrings.
    """
    IPython.display.display(
        *pipe(
            concatv([composed_function.first], composed_function.funcs),
            filter(lambda x: isinstance(x, types.LambdaType)),
            map(lambda x: ('__{}__'.format(x.__name__), x.__doc__)),
            concat,
            map(IPython.display.Markdown),
            list
        )
    )
